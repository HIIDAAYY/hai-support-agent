/**
 * Script to upload sample-hijab-shop-faq.md to Pinecone
 * under "sample-hijab-shop" namespace.
 * Run: npx tsx scripts/upload-hijab.ts
 */
...
async function main() {
  const clinicId = "sample-hijab-shop";
  const clinicName = "Toko Hijab Online Demo";
  const namespace = "sample-hijab-shop";
  const filePath = resolve(__dirname, "../data/clinics/sample-hijab-shop-faq.md");

  console.log(`🏥 Uploading Hijab Shop FAQ to namespace: ${namespace}`);
