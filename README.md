...
- **Grounded RAG answers** — semantic search over each client's KB via Pinecone (Voyage AI embeddings), with an AWS Bedrock Knowledge Base fallback. Answers cite their source; the bot won't invent facts.
...
| RAG | Pinecone vector DB · Voyage AI embeddings · AWS Bedrock (fallback) |
...
Required keys: `ANTHROPIC_API_KEY`, `PINECONE_API_KEY`, `VOYAGE_API_KEY`, `DATABASE_URL`. WhatsApp/email keys are optional for local use. See [`.env.example`](.env.example) for the full list and [`docs/QUICK_START.md`](docs/QUICK_START.md) for details.
